...
#include <ArduinoJson.h>
  ...
  #define BUDDYFS SD              // Seeed_FS instance (QSPI/SD-backed)
#endif
...
// Heap-allocated on first GIF-pack load (hardware only). The AnimatedGIF
// object is ~24 KB; keeping it off the static .bss frees that RAM for the
// display sprite. `gif` stays a drop-in name via the macro below.
static AnimatedGIF* gifPtr = nullptr;
#define gif (*gifPtr)
#ifdef BUDDY_FS
static File        gifFile;
#endif
...
// --- AnimatedGIF file callbacks (Seeed FS) — hardware only --------------
#ifdef BUDDY_FS
static void* gifOpenCb(const char* fname, int32_t* pSize) {
  gifFile = BUDDYFS.open(fname, "r");
...
bool characterInit(const char* name) {
#ifndef BUDDY_FS
  (void)name; return false;   // emulator: no filesystem — fall back to ASCII/flash-GIF
#else
  if (!BUDDYFS.begin(SDCARD_SS_PIN, SDCARD_SPI, 4000000UL)) {
    Serial.println("[char] FS mount failed");
    return false;
  }
  ...
  if (!name) {
    File d = BUDDYFS.open("/characters");
    if (d && d.isDirectory()) {
  ...
  if (!gifPtr) gifPtr = new AnimatedGIF();   // allocate the decoder on demand
...
void characterSetState(uint8_t s) {
  ...
  if (gif.open(full, gifOpenCb, gifCloseCb, gifReadCb, gifSeekCb, gifDrawCb)) {
    ...
    Serial.printf("[char] %s: %dx%d @ (%d,%d) heap=%lu\n",
      gifPaths[idx], gifW, gifH, gifX, gifY, (unsigned long)freeHeapApprox());
  } else {
